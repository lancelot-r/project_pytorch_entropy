...
def plot_boxplot_by_distribution(
    ...
    var_scipy = np.asarray(var_scipy)
    ...
    for d in dist_order:
        if d == "ALL":
            ...
            scipy_data.append(work_df["var_scipy"])
        else:
            model_data.append(
                work_df.loc[work_df["distribution"] == d, "var_model"]
            )
            scipy_data.append(
                work_df.loc[work_df["distribution"] == d, "var_scipy"]
            )
    ...
    # Legend
    ax.legend(
        [bp_model["boxes"][0], bp_scipy["boxes"][0]],
...
def main():
    ...
    scipy_mse = np.mean((df["scipy_entropy"] - df["entropy"]) ** 2)
    print("Point-estimate MSE:")
    print(f"  Model  MSE = {model_mse:.6f}")
    print(f"  SciPy  MSE = {scipy_mse:.6f}")


    # Remove rows with -inf (+inf and NaN) for scipy_entropy_boot
    n_initial = len(df)
    mask = np.isfinite(df["scipy_entropy_boot"])
    df_clean = df[mask]
    n_final = len(df_clean)
    n_removed = n_initial - n_final
    print(
        f"Removed {n_removed} rows out of {n_initial} "
        f"({100 * n_removed / n_initial:.2f}%) due to non-finite scipy_entropy_boot."
    )
    ...
    plot_var_vs_n(
        n=df["n"],
        var=(df["scipy_entropy"] - df["entropy"])**2,
        title="MSE du scipy selon n",
        ylabel="MSE",
        output_dir=base_dir,
        filename=f"{save_name}_MSEscipy_vs_n.png",
    )
    ...
    plot_boxplot_by_distribution(
        ...
        var_scipy=(df["scipy_entropy"] - df["entropy"])**2,
        title="MSE par distribution: Modèle vs SciPy",
        ylabel="MSE",
        output_dir=base_dir,
    ...
    plot_boxplot_by_distribution(
        ...
        var_scipy=df_clean["scipy_bias2"],
        title="Biais2 par distribution : Modèle vs SciPy",
        ylabel="Biais2",
        output_dir=base_dir,
    ...
    plot_boxplot_by_distribution(
        ...
        var_scipy=df_clean["scipy_var"],
        title="Variance par distribution : Modèle vs SciPy",
        ylabel="Variance",
        output_dir=base_dir,
